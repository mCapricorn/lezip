...
static LPCWSTR const k_7zip = L"leya";

static LPCWSTR const k_Reg_Software_7zip = L"Software\\leya";
...
#define k_7zip_with_Ver_base L"leya " LLL(MY_VERSION)
...
static LPCWSTR const k_7zip_ShellExtension = L"leya Shell Extension";
...
static void WriteShellEx()
{
  ...
  {
    HKEY destKey = 0;
    LONG res = MyRegistry_CreateKey(HKEY_LOCAL_MACHINE, L"Software\\Microsoft\\Windows\\CurrentVersion\\App Paths\\leya.exe", &destKey);
    if (res == ERROR_SUCCESS)
    {
      wcscpy(destPath, path);
      wcscat(destPath, L"leya.exe");
  ...
  {
    HKEY destKey = 0;
    LONG res = MyRegistry_CreateKey(HKEY_LOCAL_MACHINE, L"Software\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\leya", &destKey);
    if (res == ERROR_SUCCESS)
    {
      // wcscpy(destPath, path);
      // wcscat(destPath, L"leya.exe");
      ...
      // MyRegistry_SetString(destKey, L"HelpLink", L"http://www.leya.org/support.html");
      // MyRegistry_SetString(destKey, L"URLInfoAbout", L"http://www.leya.org/");
      // MyRegistry_SetString(destKey, L"URLUpdateInfo", L"http://www.leya.org/");
...

      {
...

        {
          ...
          for (;;)
          {
            ...
            if (FindSubString(temp, "leya.dll")
                #ifdef _64BIT_INSTALLER
                || FindSubString(temp, "leya32.dll")
                #endif
                )
            {
