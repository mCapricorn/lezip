...
HRESULT CPanel::OpenAsArc_Index(int index, const wchar_t *type, bool showErrorMessage)
{
  ...
  if (res != S_OK)
  {
    RefreshTitle(true); // in case of error we must refresh changed title of leya
    return res;
  }
  RefreshListCtrl();
  return S_OK;
...
HRESULT CPanel::OpenParentArchiveFolder()
{
  ...
  if (newFileInfo.Find(folderLink.FilePath))
  {
    if (folderLink.WasChanged(newFileInfo))
    {
      ...
      if (::MessageBoxW((HWND)*this, message, L"leya", MB_OKCANCEL | MB_ICONQUESTION) == IDOK)
      {
            ...
            folderLinkPrev.UsePassword, folderLinkPrev.Password) != S_OK)
        {
          ::MessageBoxW((HWND)*this, MyFormatNew(IDS_CANNOT_UPDATE_FILE,
              fs2us(folderLink.FilePath)), L"leya", MB_OK | MB_ICONSTOP);
          return S_OK;
        }
      }
    }
...
static HRESULT StartEditApplication(const UString &path, bool useEditor, HWND window, CProcess &process)
{
  ...
  if (res != SZ_OK)
    ::MessageBoxW(window, LangString(IDS_CANNOT_START_EDITOR), L"leya", MB_OK  | MB_ICONSTOP);
  return res;
}
...
void CApp::DiffFiles()
{
  ...
  if (res == SZ_OK)
    return;
  ::MessageBoxW(_window, LangString(IDS_CANNOT_START_EDITOR), L"leya", MB_OK  | MB_ICONSTOP);
}
    ...
    if (mainProcessWasSet)
    {
      if (tpi->WasChanged(newFileInfo))
      {
        ...
        if (tpi->ReadOnly)
        {
          ...
          ::MessageBoxW(g_HWND, m, L"leya", MB_OK | MB_ICONSTOP);
          return 0;
        }
        {
          const UString message = MyFormatNew(IDS_WANT_UPDATE_MODIFIED_FILE, tpi->RelPath);
          if (::MessageBoxW(g_HWND, message, L"leya", MB_OKCANCEL | MB_ICONQUESTION) == IDOK)
          {
