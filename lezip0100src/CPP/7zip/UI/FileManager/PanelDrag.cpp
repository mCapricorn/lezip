...
static LPCTSTR const kSvenZipSetFolderFormat = TEXT("leya::SetTargetFolder");
...
STDMETHODIMP CDropSource::QueryContinueDrag(BOOL escapePressed, DWORD keyState)
{
  ...
  if ((keyState & MK_LBUTTON) == 0)
  {
    ...
    if (needExtract)
    {
      ...
      // If you know better way to solve the problem, please notify leya developer.
...
void CPanel::OnDrag(LPNMLISTVIEW /* nmListView */)
{
...

  {
    ...
    // names variable is     USED for drag and drop from leya to Explorer or to leya archive folder.
    // names variable is NOT USED for drag and drop from leya to leya File System folder.
  ...
  We want to use MOVE_OPERATION for extracting from archive (open in leya) to Explorer:
  ...
  if DropTarget is leya window, then leya's
...
/*
  There are 2 different actions, when we drag to leya:
  1) Drag from any external program except of Explorer to "leya" FS folder.
     We want to create new archive for that operation.
  2) all another operation work as usual file COPY/MOVE
    - Drag from "leya" FS to "leya" FS.
        COPY/MOVE are supported.
    - Drag to open archive in leya.
        We want to update archive.
        We replace COPY to MOVE.
    - Drag from "leya" archive to "leya" FS.
        We replace COPY to MOVE.
*/
