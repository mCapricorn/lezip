...
#define kClassName L"leya::Panel"
...
void CPanel::MessageBox_Warning(LPCWSTR message) const
  { ::MessageBoxW((HWND)*this, message, L"leya", MB_OK | MB_ICONWARNING); }
*/
...
void CPanel::MessageBox_Error(LPCWSTR message) const
  { MessageBox_Error_Caption(message, L"leya"); }
...
void CPanel::MessageBox_Error_HRESULT(HRESULT errorCode) const
  { MessageBox_Error_HRESULT_Caption(errorCode, L"leya"); }
...
{
  return GetFolderTypeID().IsPrefixedBy_Ascii_NoCase("leya");
}
...
void CPanel::TestArchives()
{
  ...
  if (archiveFolder)
  {
    ...
    extracter.ProgressDialog.MainTitle = "leya"; // LangString(IDS_APP_TITLE);
