      ...
      UINT commandIDFirst, UINT commandIDLast, UINT flags)
{
  ...
  */
  ...
  if (ci.Cascaded.Val)
  {
    ...
    mi.StringValue = "leya"; // LangString(IDS_CONTEXT_POPUP_CAPTION);
    mi.hbmpUnchecked = bitmap;
...
static UString GetleyaPath()
{
  return fs2us(NWindows::NDLL::GetModuleDirPrefix()) + L"leya.exe";
}
...
STDMETHODIMP CZipContextMenu::InvokeCommand(LPCMINVOKECOMMANDINFO commandInfo)
{
  ...
  try
  {
    switch (cmdID)
    {
      case kOpen:
      {
        ...
        MyCreateProcess(GetleyaPath(), params);
        break;
      }
      case kExtract:
      case kExtractHere:
  ...
  {
    ::MessageBoxW(0, L"Error", L"leya", MB_ICONERROR);
  }
  return S_OK;
}
