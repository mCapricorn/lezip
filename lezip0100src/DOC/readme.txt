...
leya is a file archiver for Windows. 

leya Copyright (C) 1999-2019 Igor Pavlov.


License Info
------------

leya is free software distributed under the GNU LGPL 
...
There are two ways to compile leya binaries:
...
The final leya binaries are compiled via makefiles, that provide best 
...
nmake PLATFORM="x86" SUB_SYS_VER=1
...
Notes:
------
leya consists of COM modules (DLL files).
But leya doesn't use standard COM interfaces for creating objects.
Look at
7zip\UI\Client7z folder for example of using DLL files of leya. 
Some DLL files can use other DLL files from leya.
...
Description of leya sources package
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
7zip
  ...
  Common          Common modules for leya

  Archive         files related to archiving

  Bundle          Modules that are bundles of other modules (files)

    Alone         7za.exe: Standalone version of leya console that supports only 7z/xz/cab/zip/gzip/bzip2/tar.
    Alone7z       7zr.exe: Standalone version of leya console that supports only 7z (reduced version)
    Fm            Standalone version of leya File Manager
  ...
  UI
    ...
    Explorer      leya.dll: leya Shell extension
    Far           plugin for Far Manager
    FileManager   leya.exe: leya File Manager
    GUI           7zG.exe: leya GUI version
