...
{
  MessageBoxW(NULL, message, L"21Yasuo", MB_ICONERROR | MB_OK);
}
...
static const char * const kNoFormats = "21Yasuo cannot find the code that works with archives.";
...
static int Main2()
{
  ...
  if (commandStrings.Size() == 0)
  {
    MessageBoxW(0, L"Specify command", L"21Yasuo", 0);
    return 0;
  }
        ...
        || options.Command.IsFromUpdateGroup()))
  {
    ...
    {
      UString s ("21Yasuo cannot load module: ");
  ...
  else if (isExtractGroupCommand)
  {
	...
	if (result == S_OK)
	{
		UString str("/e,/select,");
		str += eo.OutputDir;


		ShellExecuteW(NULL, UString("open"), UString("explorer"), str, NULL, SW_SHOW);
	}
    if (result != S_OK)
    {
