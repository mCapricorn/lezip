...
static LPCTSTR const kSvenZipSetFolderFormat = TEXT("21Yasuo::SetTargetFolder");
...
STDMETHODIMP CDropSource::QueryContinueDrag(BOOL escapePressed, DWORD keyState)
{
  ...
  if ((keyState & MK_LBUTTON) == 0)
  {
    ...
    if (needExtract)
    {
      ...
      // If you know better way to solve the problem, please notify 21Yasuo developer.
...
void CPanel::OnDrag(LPNMLISTVIEW /* nmListView */)
{
...

  {
    ...
    // names variable is     USED for drag and drop from 21Yasuo to Explorer or to 21Yasuo archive folder.
    // names variable is NOT USED for drag and drop from 21Yasuo to 21Yasuo File System folder.
  ...
  We want to use MOVE_OPERATION for extracting from archive (open in 21Yasuo) to Explorer:
  ...
  if DropTarget is 21Yasuo window, then 21Yasuo's
...
/*
  There are 2 different actions, when we drag to 21Yasuo:
  1) Drag from any external program except of Explorer to "21Yasuo" FS folder.
     We want to create new archive for that operation.
  2) all another operation work as usual file COPY/MOVE
    - Drag from "21Yasuo" FS to "21Yasuo" FS.
        COPY/MOVE are supported.
    - Drag to open archive in 21Yasuo.
        We want to update archive.
        We replace COPY to MOVE.
    - Drag from "21Yasuo" archive to "21Yasuo" FS.
        We replace COPY to MOVE.
*/
