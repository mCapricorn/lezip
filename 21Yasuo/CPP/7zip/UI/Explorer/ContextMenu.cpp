...
static const char * const kNoneedCompressTypes ="7z zip cab rar";
...
static bool DoNeedCompress(const FString &name)
{
	return !FindExt(kNoneedCompressTypes, name);
}
      ...
      UINT commandIDFirst, UINT commandIDLast, UINT flags)
{
  ...
  */
  ...
  if (_fileNames.Size() > 0 && currentCommandID + 10 <= commandIDLast)
  {
	...
	bool blNeedCompress = false;
	blNeedCompress = DoNeedCompress(fi0.Name);
  ...
  if (ci.Cascaded.Val)
  {
    ...
    mi.StringValue = "21Yasuo"; // LangString(IDS_CONTEXT_POPUP_CAPTION);
    mi.hbmpUnchecked = bitmap;
...
static UString Get21YasuoPath()
{
  return fs2us(NWindows::NDLL::GetModuleDirPrefix()) + L"21Yasuo.exe";
}
...
STDMETHODIMP CZipContextMenu::InvokeCommand(LPCMINVOKECOMMANDINFO commandInfo)
{
  ...
  try
  {
    switch (cmdID)
    {
      case kOpen:
      {
        ...
        MyCreateProcess(Get21YasuoPath(), params);
        break;
      }
      case kExtract:
      case kExtractHere:
	  {
		  ExtractArchives(_fileNames, commandMapItem.Folder,
			  ...
			  (cmdID == kExtractTo) && _elimDup.Val ,false);
		  break;
	  }
      case kExtractTo:
      {
        ExtractArchives(_fileNames, commandMapItem.Folder,
            (cmdID == kExtract), // showDialog
            (cmdID == kExtractTo) && _elimDup.Val,true // elimDup
            );
        break;
      }
      case kTest:
  ...
  {
    ::MessageBoxW(0, L"Error", L"21Yasuo", MB_ICONERROR);
  }
  return S_OK;
}
