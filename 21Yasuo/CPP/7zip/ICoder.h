...
/*
  ...
  Parameters:
    ...
    if (inSize != NULL)
    {
      Encoders in 21Yasuo ignore (inSize).
      Decoder can use (*inSize) to check that stream was decoded correctly.
      Some decoder in 21Yasuo check it, if (full_decoding mode was set via ICompressSetFinishMode)
    }
    ...
    if (outSize != NULL)
    {
      Encoders in 21Yasuo ignore (outSize).
