/* 7zipInstall.c - 21Yasuo Installer
2019-02-19 : Zhi Yun : Public domain */
...
static LPCWSTR const k_7zip = L"21Yasuo";

static LPCWSTR const k_Reg_Software_7zip = L"Software\\21Yasuo";
...
static LPCWSTR const k_7zip_ShellExtension = L"21Yasuo Shell Extension";
...
static void WriteCLSID()
{
  ...
  if (res == ERROR_SUCCESS)
  {
    ...
    wcscpy(destPath, path);
    wcscat(destPath, L"21Yasuo32.dll");
...
static void WriteShellEx()
{
  ...
  {
    HKEY destKey = 0;
    LONG res = MyRegistry_CreateKey(HKEY_LOCAL_MACHINE, L"Software\\Microsoft\\Windows\\CurrentVersion\\App Paths\\21Yasuo.exe", &destKey);
    if (res == ERROR_SUCCESS)
    {
      wcscpy(destPath, path);
      wcscat(destPath, L"21Yasuo.exe");
  ...
  {
    HKEY destKey = 0;
    LONG res = MyRegistry_CreateKey(HKEY_LOCAL_MACHINE, L"Software\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\21Yasuo", &destKey);
    if (res == ERROR_SUCCESS)
    {
      // wcscpy(destPath, path);
      // wcscat(destPath, L"21Yasuo.exe");
...

      {
...

        {
          ...
          for (;;)
          {
            ...
            if (FindSubString(temp, "21Yasuo.dll")
                #ifdef _64BIT_INSTALLER
                || FindSubString(temp, "21Yasuo32.dll")
                #endif
                )
            {
